...
| M4-3 | **Historical value chart.** _Shipped 2026-08-10, real snapshots only._ The reconstruction line is deliberately absent from v1: every point drawn is a reading, days never recorded widen their segment (date-proportional axis), unpriceable days break the line. The second series gets its own decision once there is a real chart to compare against (`M4_PLAN.md` §5).                     | L      | The reconstruction caveat is non-negotiable — see kickoff principle "do not claim a value is exact when it is estimated".    |
