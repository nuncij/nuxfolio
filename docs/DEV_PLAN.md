...
`DECISIONS.md` (ADR-001…034), `PROVIDERS.md`, `REVIEW_LOG.md` (seventeen
...
| #    | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Effort | Notes                                                                                                                        |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------- |
| M4-1 | **Choose and introduce the store.** _Shipped 2026-08-10 as SQLite through `node:sqlite` (ADR-033), not the Postgres this row predicted — the box was measured first. Zero new dependencies; one table._                                                                                                                                                                                                                                                                                            | M      | Deliberately _after_ M3, so the store serves a proven need.                                                                  |
| M4-2 | **Portfolio snapshots.** _Shipped 2026-08-10._ A daily systemd timer on the host posts to `/api/snapshot` (keyed, 404 without it); one row per chain per UTC day for a fixed tracked list from the environment — not "each uncached load", and no payload column: a snapshot is a total, two counts and a coverage flag, ~2 KB of a 195 KB response. Retention is "keep it" (`M4_PLAN.md` §6).                                                                                                     | M      | The tracked list is env, not code — the repo is public and the owner's addresses are not (M4 review round 14).               |
| M4-3 | **Historical value chart.** _Shipped 2026-08-10, real snapshots only._ The reconstruction line is deliberately absent from v1: every point drawn is a reading, days never recorded widen their segment (date-proportional axis), unpriceable days break the line. The second series was measured against 24 days of real readings on 2026-09-03 and **declined** — it was ~9 % wrong for a wallet whose holdings had changed, with nothing to tell that case from a wallet that had not (ADR-034). | L      | The reconstruction caveat is non-negotiable — see kickoff principle "do not claim a value is exact when it is estimated".    |
| M4-4 | **Shared cache / rate-limit store.** Only if deployment scales past one instance (ADR-007). Same interfaces, Redis/Upstash implementation.                                                                                                                                                                                                                                                                                                                                                         | M      | Explicitly deferred until measured need. Still deferred; ADR-033 names one instance as the trigger to revisit the store too. |
...
| M5-5 | ✅ **`PositionProvider` interface — evaluated with two adapters in hand, and declined.** The three-value read status was extracted (seven copies across five files); the interface itself would have hidden the one difference that matters, which is that the two adapters price and schedule differently on purpose. | —      | Decided 2026-08-08. ADR-031.     |
...
**M5-3 followed on 2026-08-08**, closing the one open item that was a defect rather than
a feature: the page now says Aave v3 is the only protocol it reads — in the lending
...
**M5-5 was then settled the way it was meant to be** — with two adapters in hand rather
than none. The answer was no: the seven copies of the read status became one shared type,
and the interface itself was declined, because the two adapters differ in exactly the
place an interface would have to paper over (ADR-031). The evidence is that Convex was
written without it and is shorter for it.

That leaves **M5-7** alone, and it is gated rather than pending: it says to evaluate an
indexer shortcut _after three first-party adapters exist_, and there are two.
